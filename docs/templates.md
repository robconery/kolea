...
├── index.hbs             ← required: the archive (home and page 2, 3…)
├── post.hbs              ← required: one post
├── home.hbs              ← optional: page 1 of the archive only
...
|---|---|---|
| `/` | `home` → `index` | `posts`, `pagination` |
| `/page/2` | `index` | `posts`, `pagination` |
...
A post's **first tag is its topic**, and the topic is its URL. `/feed.xml`,
...
| Value | What it is |
|---|---|
| `@site.title`, `@site.description` | The site's name and tagline |
| `@site.url` | The site's origin, e.g. `https://a.bigmachine.io` |
| `@site.author` | The author's name |
| `@site.navigation` | Home plus the busiest topics: each has `label`, `url`, `hue`, `hue_cool` |
...
2. **Subscribe forms post to `@site.signup_action`**, with the address in a field named `email`
...
3. **Readable with JavaScript off.** If content is hidden for a reveal animation, hide it only
...
4. **Respect `prefers-reduced-motion`.** All animation off.
5. **No trackers, no third-party scripts** beyond fonts. The list is owned; the reading is too.
6. **Assets via `{{asset}}`** so they are cache-busted on every upload.
7. **Every page type renders**, including the error page, search with no results, a
   topic with one post, and a post with no image. Most of your archive has no images.
...
index.hbs, post.hbs, tag.hbs, search.hbs, subscribe.hbs, error.hbs, any partials,
